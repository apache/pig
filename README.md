...
Pig compiles these dataflow programs into (sequences of) map-reduce or 
[Apache Tez](https://tez.apache.org/) jobs and executes them using Hadoop. It is 
also possible to execute Pig Latin programs in a "local" mode (without Hadoop 
cluster), in which case all processing takes place in a single local JVM. 
