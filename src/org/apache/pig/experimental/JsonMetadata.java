...
package org.apache.pig.experimental;
...
public class JsonMetadata implements LoadMetadata, StoreMetadata {
    ...
    public JsonMetadata(String fullPath, DataStorage storage) {
        this.resourcePath = fullPath;
     ...
     */
    protected Set<ElementDescriptor> findMetaFile(String path, String prefix, DataStorage storage) 
        throws IOException {
    ...
    public ResourceSchema getSchema(String location, Configuration conf) throws IOException {
        ...
        try {
            schemaFileSet = findMetaFile(resourcePath, schemaFileName, storage );
        } catch (IOException e) {
            log.warn("Could not find schema file for "+resourcePath); 
            return null;
        }
        ...
        try {
            ...
            log.warn("Unable to load Resource Schema for "+resourcePath);
            e.printStackTrace();
     ...
     * This method suppresses (and logs) errors if they are encountered.
     * @see org.apache.pig.experimental.LoadMetadata#getStatistics(String, Configuration)
     */
    @Override
    public ResourceStatistics getStatistics(String location, Configuration conf) throws IOException {
        ...
        try {
            statFileSet = findMetaFile(resourcePath, statFileName, storage);
        } catch (IOException e) {
            log.warn("could not fine stat file for "+resourcePath);
            return null;
        }
        ...
        if (!statFileSet.isEmpty()) {
            ...
            log.warn("Could not find stat file for "+resourcePath);
            return null;
        }
        ...
        try {
            ...
            log.warn("Unable to load Resource Statistics for "+resourcePath);
            e.printStackTrace();
        } catch (JsonMappingException e) {
            log.warn("Unable to load Resource Statistics for "+resourcePath);
            e.printStackTrace();
    ...
    @Override
    public void setParitionFilter(OperatorPlan<Operator<PlanVisitor<?,?>>> plan)
            throws IOException {
        // TODO Auto-generated method stub
    ...
    public void setStatistics(ResourceStatistics stats, String location, Configuration conf) throws IOException {
        ElementDescriptor statFilePath = storage.asElement(resourcePath, statFileName);
        if(!statFilePath.exists() && stats != null) {
            try {
                new ObjectMapper().writeValue(statFilePath.create(), stats);                    
            } catch (JsonGenerationException e) {
                log.warn("Unable to write Resource Statistics for "+resourcePath);
                e.printStackTrace();
            } catch (JsonMappingException e) {
                log.warn("Unable to write Resource Statistics for "+resourcePath);
                e.printStackTrace();
    ...
    public void setSchema(ResourceSchema schema, String location, Configuration conf) throws IOException {
        ElementDescriptor schemaFilePath = storage.asElement(resourcePath, schemaFileName);
        ...
        if (printHeaders) {
            ElementDescriptor headerFilePath = storage.asElement(resourcePath, headerFileName);
