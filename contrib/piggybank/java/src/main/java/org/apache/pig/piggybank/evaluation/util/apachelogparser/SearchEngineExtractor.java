...
public class SearchEngineExtractor extends EvalFunc<DataAtom> {
    ...
    public void exec(Tuple input, DataAtom output) {
        String referer = input.getAtomField(0).strval();

        if (referer == null || referer.length() < 4)
            return;
