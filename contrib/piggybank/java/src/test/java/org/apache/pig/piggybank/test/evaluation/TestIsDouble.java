...
public class TestIsDouble {
	private static final EvalFunc<Boolean> isDouble = new IsDouble();
    private static Tuple testTuple = TupleFactory.getInstance().newTuple(1);
    
    @Test
    public void testIsDouble() throws IOException {
        testTuple.set(0, "123432423441.12");
        assertTrue(isDouble.exec(testTuple));

        testTuple.set(0, "1aaannn234");
        assertFalse(isDouble.exec(testTuple));
