 ...
 * the License.
 */
package org.apache.hadoop.owl.schema;
 ...
 */
public class Schema {
    //implements Comparable<Schema>, Writable {
    ...
    public static class ColumnSchema {
        ...
        public Schema schema;  //subschema
        ...
        public static boolean equals(ColumnSchema fschema, ColumnSchema fother) {
            ...
            else if (fschema.name == null || ! fschema.name.equals(fother.name)) {
                return false;
            }

            if (ColumnType.isSchemaType(fschema.type)) {

                if( fschema.type == ColumnType.MAP && fother.type == ColumnType.MAP ) {
                    //Allow maps where one has map schema with empty name and another has map with null name
                    if( fschema.schema != null && fother.schema != null ) {
                        if( fschema.schema.getNumColumns() == 1 && fother.schema.getNumColumns() == 1 ) {
                            if( fschema.schema.getColumn(0).name == null || fschema.schema.getColumn(0).name.equals("") )
                                if( fother.schema.getColumn(0).name == null || fother.schema.getColumn(0).name.equals("") ) {
                                    return true;
                                }
                        }
                    }
                ...
                if (!(fschema.schema == null && fother.schema == null)) {
                    ...
                    if (fschema.schema == null || ! fschema.schema.equals(fother.schema)) {
                        return false;
                    }
                }
            }
    ...
    public ArrayList<ColumnSchema> getColumnSchemas(){
        return this.mFields;
    }
    /**
    ...
    public boolean equals(Object obj) {
        ...
        while (i.hasNext()) {
            ...
            else if (myFs.name == null || !myFs.name.equals(otherFs.name)) {
                return false;
            }
  ...
  public void write(DataOutput out) throws IOException {
    ...
    private void init(String[] columnNames) throws ParseException {
        ...
        for (int i = 0; i < columnNames.length; i++) {
            ...
            merged.append(columnNames[i]);
        }
        TableSchemaParser parser = new TableSchemaParser(new StringReader(merged.toString()));
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            if (fs == null)
                ...
                if (!ColumnSchema.equals(fs, otherfs))
                    throw new ParseException("Different types of column "+fs.name+" in schema union");
            }
        }
    }  
